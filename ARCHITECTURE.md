...
still yields geometry only. See* glTF PBR material import (Step 16) *below.)* *(Step 25 adds
`loadModelHierarchy`, which imports a full glTF node tree as a `Node` subtree — one child per
mesh primitive/material, node matrices decomposed to TRS via `Transform::fromMatrix`, shared
textures uploaded once — leaving `loadModel` as the single-mesh path.)*
