...
> **Current status — Step 14:** adds a **skybox &amp; environment cubemap**. The scene is now wrapped
> in a **cubemap sky** — six textures forming a cube, sampled by *direction* rather than a 2D
> coordinate. It's drawn as a unit cube around the camera, with the view matrix's translation stripped
> (so the sky stays infinitely far) and its depth pinned to the far plane (so it sits behind every
> object without hiding any). A procedural **day sky** is generated by
> [`tools/gen_skybox.py`](tools/gen_skybox.py); the cubemap plumbing is the prerequisite for
> image-based lighting (Step 15). Toggle it at runtime with `8`.
>
> **Controls:** `W`/`A`/`S`/`D` move, `E`/`Q` up/down, mouse to look, `Esc` to quit.
> Post-processing: `1` tone-map, `2` bloom, `3` FXAA, `4` vignette, `[` / `]` exposure.
> Lights: `5` point lights, `6` spot, `7` sun. Environment: `8` skybox.
